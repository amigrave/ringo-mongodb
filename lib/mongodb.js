module.exports = (function(mongo) {
    ...
    var files = java.io.File(module.resolve('../jars')).listFiles();
    for (var i = 0; i < files.length; i++) {
        var file = files[i].toString();
        if (file.match(/\.jar$/) && !addToClasspath(file)) {
    ...
    javaExtend(mongo.MongoClient, {
        ...
        toString: function() {
            var uri = 'mongodb://' + this.__javaObj.address.getHost();
            return "[mongo.MongoClient: " + uri + "]";
        },
    });
    ...
    javaExtend(mongo.MongoDB, {
        ...
        toString: function() {
            var uri = 'mongodb://' + this.__javaObj.mongo.address.getHost();
            uri += '/' + this.name;
            return "[mongo.MongoDB: " + uri + "]";
        },
    ...
    javaExtend(mongo.Collection, {
        ...
        toString: function() {
            var uri = 'mongodb://' + this.__javaObj.DB.mongo.address.getHost();
            uri += '/' + this.__javaObj.DB.name + '::' + this.name;
            return "[mongo.Collection: " + uri + "]";
        },
        ...
        findOne: function(query) {
            ...
            return new mongo.Document(doc, this);
        },
        insert: function(obj) {
    ...
    // Document
    mongo.Document = function Document(doc, collection) {
        this.__javaObj = doc;
    ...
    javaExtend(mongo.Document, {
        ...
        toString: function() {
            var uri = 'mongodb://' + this.collection.__javaObj.DB.mongo.address.getHost();
            uri += '/' + this.collection.__javaObj.DB.name + '::' + this.collection.name;
            uri += '#' + this.data._id.toString();
            return "[mongo.Document: " + uri + "]";
        },
        toJSON: function() {
